...
def test_natural_unit_params_are_normalized(synthetic_checkpoint):
    ...
    lo, hi = EQ_PLUGIN_CONFIGS["channel_eq"]["ranges"]["hf_gain"]

    eng = NeuralEQEngine(synthetic_checkpoint(), block_size=BLOCK, crossfade_blocks=0)
    eng.set_params({"lf_gain": 0.0, "lmf_gain": 0.0, "hmf_gain": 0.0, "hf_gain": hi})
    fir_at_max = eng._current_fir.copy()

    eng.set_params({"lf_gain": 0.0, "lmf_gain": 0.0, "hmf_gain": 0.0, "hf_gain": lo})
    fir_at_min = eng._current_fir.copy()

    # The encoder is monotone, so the two extremes must not collapse together.
    assert not np.allclose(fir_at_max, fir_at_min)
    assert EQ_PLUGIN_CONFIGS["channel_eq"]["params"][-1] == "hf_gain"